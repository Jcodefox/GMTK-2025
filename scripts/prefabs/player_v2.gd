...
@export var jump_gravity_multiplier: float = 0.5
...
func _physics_process(delta: float) -> void:
	...
	if (
		(time_since_jump_attempt < jump_buffer_time) and 
		(time_since_on_floor < coyote_time or jumps_left > 1)
	):
		jumps_left -= 1
		velocity.y = -1 * jump_init_velocity
